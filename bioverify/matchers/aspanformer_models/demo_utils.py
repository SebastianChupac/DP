...
def draw_match(img1, img2, corr1, corr2, inlier=[True], color=None, radius1=1, radius2=1, resize=None):
    if resize is not None:
        scale1 = [img1.shape[1] / resize[0], img1.shape[0] / resize[1]]
        scale2 = [img2.shape[1] / resize[0], img2.shape[0] / resize[1]]
        img1 = cv2.resize(img1, resize, interpolation=cv2.INTER_AREA)
        img2 = cv2.resize(img2, resize, interpolation=cv2.INTER_AREA)
        corr1 = corr1 / np.asarray(scale1)[np.newaxis]
        corr2 = corr2 / np.asarray(scale2)[np.newaxis]
    ...
    if len(color) == 1:
        ...
        for i in range(len(corr1)):
            left_x = int(corr1[i][0])
            left_y = int(corr1[i][1])
            right_x = int(corr2[i][0] + img1.shape[1])
            right_y = int(corr2[i][1])
